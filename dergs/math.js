...
function render(){
	...
	rendering = rendering + render_rep(1) + "</table></td></tr><tr><td><button id='rbutt' class='big_butt' type='button' onclick='rate()'>Rate Difficulty</button></td></tr>";
	tots();
	...
	for(var i=1; i<11; i++){
		if(i == r){
			rep_opt_string += "<OPTION SELECTED>" + i + "</OPTION>";
		}
		else{
			rep_opt_string += "<OPTION>" + i + "</OPTION>";
		}
	}
	rendering += rep_opt_string + "</SELECT></FORM></td></tr></table></td></tr></table>";
...
function update2k(){
	...
	document.location = 'workout.html?' + pid + '&Best2k=' + y.getElementsByTagName("option")[x].value + '&reps=' + reps;
}
...
function update_reps(){
	...
	if(x < z){
		...
		for(var i = z; i<x; i++){
			rendering = rendering + render_rep(i+1);
		}
		z = '#' + z + '_intv' + (len - 1);
		$(z).after(rendering);
	}
...
function rate(){
	...
	text += '<tr><td>Difficulty:</td><td>1</td><td>2</td><td>3</td><td>4</td><td>5</td><td><button class="big_butt" type="button" onclick="submitRating()">Submit</button></td></tr>';
